...
async def cb_templates_menu(cq: CallbackQuery):
    ...
    if not templates:
        ...
        lines = ["📋 <b>Subscription Templates</b>\n"]
        for t in templates:
    ...
    builder.adjust(1)
    builder.row(InlineKeyboardButton(text="➕ Create template", callback_data="tmpl_create"))
...
async def cb_template_detail(cq: CallbackQuery):
    ...
    text = (
        f"📋 <b>{t['label']}</b>{star}\n"
        f"Name: <code>{t['name']}</code>\n"
        f"ID: {t['id']}\n\n"
        f"Config JSON preview (first 300 chars):\n"
        f"<pre>{t['config_json'][:300]}...</pre>"
    )

    builder = InlineKeyboardBuilder()
    if not t.get("is_default"):
        builder.button(text="⭐ Set as default", callback_data=f"tmpl_setdef_{tid}")
    builder.button(text="📄 View full JSON", callback_data=f"tmpl_json_{tid}")
    if not t.get("is_default"):
        builder.button(text="🗑 Delete", callback_data=f"tmpl_delete_{tid}")
    builder.adjust(1)
    builder.row(InlineKeyboardButton(text="⬅️ Back", callback_data="menu_templates"))
...
async def cb_template_json(cq: CallbackQuery):
    ...
    await cq.message.answer_document(file, caption=f"📄 Template: <b>{t['label']}</b>", parse_mode="HTML")
    await cq.answer()
...
async def cb_template_setdefault(cq: CallbackQuery):
    ...
    try:
        ...
        await cq.answer(f"✅ '{t['label']}' is now the default template")
    except APIError as e:
        await cq.answer(f"❌ {e.detail}", show_alert=True)
        return
    # Refresh list
...
async def fsm_tmpl_name(msg: Message, state: FSMContext):
    ...
    await msg.answer("Step 2/3: Enter a display label (e.g. <b>My OpenWRT Router</b>):", parse_mode="HTML")
...
async def fsm_tmpl_json(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(f"❌ Invalid JSON: {e}\n\nTry again:")
        return
...
async def cb_client_template(cq: CallbackQuery):
    ...
    if current_tid is not None:
        builder.button(text="↩️ Use default template", callback_data=f"client_set_tmpl_{cid}_0")
    builder.adjust(1)
    builder.row(InlineKeyboardButton(text="⬅️ Cancel", callback_data=f"client_detail_{cid}"))
...
async def cb_client_set_template(cq: CallbackQuery):
    ...
    try:
        ...
        label = "default" if new_tid is None else str(new_tid)
        try:
            templates = await client_tmpl_api.list()
            ...
            pass
        await cq.answer(f"✅ Template set: {label}")
    except APIError as e:
        await cq.answer(f"❌ {e.detail}", show_alert=True)
        return
